# Agustina Bussi (Agus)

## Tu rol

**Investigadora · Coordinación pedagógica y de producto · UX.**

Eres quien hace el puente entre el corazón pedagógico, el desarrollo de producto y la validación con docentes reales. En el proyecto eres la responsable de que el curso piloto sea pedagógicamente sólido y que el cuaderno digital ofrezca una experiencia de uso defendible frente a un panel académico.

## Lo que haces

### En la fase de postulación (mayo–junio 2026)

- **Validar el anclaje pedagógico del proyecto** (las cuatro categorías de marcado, el modelo de niveles, la lógica del onboarding)
- **Definir el contenido curricular del curso piloto** alineado al UNESCO AI Competency Framework for Teachers (2024)
- **Documentar la decisión técnica del versionado del notebook** (Architecture Decision Record con Steven)
- **Revisar y aportar input** a la sección 4.5.1 (cuaderno como dispositivo) del formulario ANII

### En la fase de ejecución (oct 2026 – jun 2027)

- Lead pedagógica del curso piloto: estructura, contenidos, ejercicios, mini-retos
- Apoyo en la adaptación de la escala Tschannen-Moran al dominio IA (con panel de expertos)
- Coordinación con Steven para que el cuaderno técnico capture lo que pedagógicamente importa
- Co-autoría del documento metodológico abierto (sección de operacionalización pedagógica)
- Apoyo en codificación cualitativa de comments y mini-retos

## Lo que decides

| Decisión | Plazo | Estado |
|----------|-------|--------|
| Contenido curricular del curso piloto (mapeo a UNESCO AICFT 2024) | 31 may 2026 | **Pendiente** |
| Diseño de mini-retos quincenales (rúbrica de "completado") | 31 may 2026 | Pendiente |
| Versión final del onboarding por capas (stickers → comments → notas de voz) | 31 may 2026 | Decidido (ratificar para v2.1) |
| Definición pedagógica del 9° arquetipo | M0 oct 2026 | Pendiente |
| Co-decisión: si se adopta la 5ª categoría (Disenso) o se mantiene en 4 | 31 may 2026 | Pendiente |

## Tensiones que tienes que ayudar a resolver

!!! abstract "Tensión filosófica del producto"
    Notebook personal vs LMS secuencial vs Red social tipo Reddit. El proyecto ANII se ejecuta sobre **versión congelada del bundle**, así que esta tensión no bloquea la postulación, pero afecta cómo se narra el cuaderno en el formulario (sección 4.5.1).

!!! abstract "Riesgo de infantilización"
    Stickers kawaii + coleccionismo pueden percibirse como condescendientes para docentes con experiencia. Esto es algo que el Discovery local con docentes RD debería validar antes del estudio.

!!! abstract "Heterogeneidad de los arquetipos"
    Algunos son comportamentales (Maker, Narrador) y otros aspiracionales (Catalizador, Tejedor). Esto puede bajar la validez discriminante del SJT-POV. Para el proyecto ANII, el SJT se aplica solo como caracterización descriptiva (no como predictor), pero conviene tener tu opinión sobre si esa decisión es defendible.

## Riesgos que dependen de ti

!!! danger "Validez del curso piloto"
    El itinerario formativo de 10 semanas es el contexto donde se observa todo. Si el contenido pedagógico es flojo o no se alinea bien al UNESCO AICFT 2024, el revisor académico va a cuestionar todo el diseño.

    **Mitigación:** documento de mapeo curso × competencias UNESCO antes del 31 may.

!!! warning "Adaptación de Tschannen-Moran a IA"
    Necesitamos panel de expertos en M1 para revisar la adaptación. Tu rol: coordinar con Berenice la convocatoria del panel.

## Lo que necesitamos de ti esta semana

1. **Confirmar contenido del curso piloto** y mapeo a UNESCO AI Competency Framework for Teachers (2024)
2. **Validar el modelo de 5 niveles** desde la perspectiva pedagógica — ¿hay confusión entre "intervención reflexiva" y "transferencia"?
3. **Revisar el borrador del documento metodológico abierto** que la propuesta promete entregar bajo CC-BY: ¿qué incluye, qué se reserva?
4. **Opinión sobre el 9° arquetipo**: ¿definir antes del estudio o asumir 8 + "otros" durante el piloto?

## Tu interlocutor principal

**Berenice** para alineación pedagógico-científica; **Steven** para cómo se traduce lo pedagógico en captura técnica del log; **Sebas** para integración operativa con el curso real que se va a aplicar en RD.

---

[:material-arrow-left-circle: Ver todos los roles](roles.md) · [:material-arrow-right-circle: Sigue: Steven](steven.md)
